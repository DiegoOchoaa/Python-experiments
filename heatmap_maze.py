...
clock = pygame.time.Clock()
pygame.display.set_caption("depht first search maze generetor"  )
...
def algorithm_Astar(draw, grid, start, end, vis, frame_rate, delay):
    cn = 0
    count = 0
...
def main(screen, width):
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:                
                if event.key == pygame.K_SPACE:
                     ...
                     if not algorithm_Astar(lambda: draw(screen, grid ,ROWS, width), grid, start_pos, end, vis, frame_rate, delay):  #added for comical propouses you can put only algorithm(lambda: draw(screen, grid ,ROWS, width), grid, start, end): and it will work
        ...
        if pygame.mouse.get_pressed()[0]: #LEFT   # totally optional does not need to be here
               
                pos = pygame.mouse.get_pos()
                row, col = get_clicked_pos(pos, ROWS, width)

                clicked_cell = grid[row][col] 
                clicked_cell.make_open()
